...
export const ENTERPRISE_EXTRA_VOICES = [
...
export const ENTERPRISE_VOICES = [...PREMIUM_VOICES, ...ENTERPRISE_EXTRA_VOICES];

/**
 * @param {'free'|'premium'|'enterprise'} tier
 * @returns {{id: string, label: string}[]} voices selectable on that tier (empty for free)
 */
export function getVoiceChoicesForTier(tier) {
 ...
 * @param {'free'|'premium'|'enterprise'} tier
 */
export function isVoiceAllowedForTier(voiceId, tier) {
 ...
 */
export function resolveVoiceForGuild({ tier, savedVoiceId }) {
