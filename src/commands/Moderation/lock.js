...
export default {
  ...
  async execute(interaction, config, client) {
    ...
    if (sub === "unlock") {
      return executeUnlock(interaction, config, client);
    }
    return executeLock(interaction, config, client);
  },
};

// Original lock.js logic, unchanged.
async function executeLock(interaction, config, client) {
...
// Original unlock.js logic, unchanged.
async function executeUnlock(interaction, config, client) {
  const deferSuccess = await InteractionHelper.safeDefer(interaction);
  if (!deferSuccess) {
