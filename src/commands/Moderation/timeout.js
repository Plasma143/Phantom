...
export default {
  data: new SlashCommandBuilder()
    .setName("timeout")
    .setDescription("Timeout a user, or remove an active timeout.")
  ...
  async execute(interaction, config, client) {
    ...
    if (sub === "remove") {
      return executeRemove(interaction, config, client);
    }
    return executeSet(interaction, config, client);
  },
};

// Original timeout.js logic, unchanged.
async function executeSet(interaction, config, client) {
...
// Original untimeout.js logic, unchanged.
async function executeRemove(interaction, config, client) {
  const deferSuccess = await InteractionHelper.safeDefer(interaction);
  if (!deferSuccess) {
