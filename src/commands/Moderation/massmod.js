...
export default {
  data: new SlashCommandBuilder()
    .setName("massmod")
    .setDescription("Bulk moderation actions for multiple users at once.")
  ...
  async execute(interaction, config, client) {
    ...
    if (sub === "kick") {
      return executeMasskick(interaction, config, client);
    }
    return executeMassban(interaction, config, client);
  },
};

// Original massban.js logic, unchanged (including its own 3-per-60s rate limit, separate from the global abuse-protection layer).
async function executeMassban(interaction, config, client) {
...
// Original masskick.js logic, unchanged (including its own 3-per-60s rate limit, separate from the global abuse-protection layer).
async function executeMasskick(interaction, config, client) {
  const deferSuccess = await InteractionHelper.safeDefer(interaction);
  if (!deferSuccess) {
